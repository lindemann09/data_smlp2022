...
# Definition of congruency parameter

Congruency is defined as $$c =  \frac{v}{3}  \cdot \frac{n-5}{4} = \frac{v(n-5)}{12}$$

```{r def congruency}
congruency_parameter = function(num_size, variable) {
  n = num_size
  v = variable
  return(v*(n-5)/12)
}
...
```{r}
raw = read_csv("WBL16_raw_data.csv", na =c("NA", "None"), show_col_types = FALSE) %>%
...
data = raw %>%
    ...
    mutate(
      ...
      SiC = congruency_parameter(digit, size_rc),
      SpC = congruency_parameter(digit, pos_rc)
      )
```

## Determine congruency of previous trails (Gratton effect)
...
# set first "previous congruency" of each subject to NaN
...
glimpse(data)
xtabs(~data$Subject)
xtabs(~pos_rc+size_rc, data=data)
```

## Select data

```{r}
rt.data = filter(data, error==0, resp !="no", rt>200)
```
...
# testing random slope for spatial congruency
...
# testing random slope for previous size congruency
...
# testing random slope for previous position congruency
